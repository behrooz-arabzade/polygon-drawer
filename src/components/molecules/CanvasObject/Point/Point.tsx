...
const Point: FC<IPoint> = ({ index, pointX, pointY, isDrawing, polygonPointSize, onPointMove, onFirstPointClick }) => {
  //...... local states ......//
  ...
  const onPointDragEnd = (index: number) => (event: KonvaEventObject<MouseEvent>) => {
    event.target.scale({ x: 1, y: 1 });
  ...
  const onDragMove = (index: number) => (event: KonvaEventObject<MouseEvent>) => {
    const stage = event.target.getStage();
    const mousePos = getPoint2d(getMousePos(stage), zoom);
